...
trait FU:
  ...
  def exactlyOne[F[_]: ApplicativeThrow, A](msg: String)(l: List[A]): F[A] =
    val err: IllegalStateException = IllegalStateException(s"Requires List Size 1 but  ${l.length}: $msg")
    if l.length > 1 then ApplicativeThrow[F].raiseError[A](err)
  ...
  def required[F[_]: ApplicativeThrow, A](msg: String)(o: Option[A]): F[A] = optionToError(o)(Throwable(s"Required Value Missing: $msg"))
  ...
  def nonEmptyListIO[F[_]: ApplicativeThrow, A](msg: String)(l: List[A]): F[NonEmptyList[A]] =
    optionToError(NonEmptyList.fromList(l))(Throwable(s"NEL Requires List of 1+ $msg"))
